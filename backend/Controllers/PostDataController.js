...
const postdata = async (req, res) => {
  try {
    ...
    let semesterExists = await UserModel.findOne({ user: user._id, semesterNumber });

    if (semesterExists) {
      return res.status(409).json({ error: "Semester already exists. Cannot add duplicate semester." });
    }
